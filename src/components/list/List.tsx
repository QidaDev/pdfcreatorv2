  ...
  return (
    <View style={tw(cleanStyle(`flex flex-col gap-2 mb-${marginBottom} mt-${marginTop} ml-${marginLeft} mr-${marginRight}`))}>
      {items?.map((item, index) => {
        return (
          <View wrap={false} key={index} style={tw(cleanStyle(`flex flex-row gap-2 ${haveCheckbox ? '' : ''} ${fontSize ? `text-[${fontSize}]` : ''} ${lineHeight ? `leading-[${lineHeight}]` : ''}  mb-${itemsSeparation}`))}>
