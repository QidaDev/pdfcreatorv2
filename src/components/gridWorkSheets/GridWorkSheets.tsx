        ...
        {items.map((item, index) => {
          return (
            <View
              ...
              style={tw(cleanStyle(`w-[47%] ${backgroundColor} border-b-4 p-3 ${borderColor} mb-6`))}
            >
              <Text style={tw(cleanStyle(`text-[13px] leading-6 font-bold mt-3`))}>
                {renderText(item.text)}
              </Text>

              <Text style={tw(cleanStyle(`leading-6 mt-3 text-[12px]`))}>
                {renderText(item.description)}
              </Text>
              <View>
                <Link
                  style={tw(cleanStyle(`${buttonColor} w-32 py-2 px-3 rounded-full my-6 text-white no-underline text-base flex flex-row items-center justify-between`))}
                  src={item.linkSrc}
                >
                  <Text>{renderText(item.linkText)}</Text>
                  <RightArrowSvg
                    style={tw(cleanStyle(`ml-4`))}
                    width="18px"
                    height="18px"
                    color={`#ffffff`}
                  />
