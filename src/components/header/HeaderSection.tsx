      ...
      <View fixed={(showHeaderOnEveryPage && showSectionTitleOnEveryPage)} style={tw(cleanStyle(`mt-[-8px]`))}>
        <View style={tw(cleanStyle(`flex flex-row justify-between items-center mb-6 ml-[-${pagePadding.left}] mr-[-${pagePadding.right}]`))}>
          <View style={tw(cleanStyle(`flex flex-row w-[50%] pl-12 py-8 bg-${color}`))}>
            <View
              style={tw(cleanStyle(`flex flex-row items-center text-[20px] font-medium tracking-wide`))}
            >
              <Text style={tw(cleanStyle('text-black'))}>{renderText(title)}</Text>
            </View>
          </View>
          <View style={tw(cleanStyle('w-[45%] text-[12px] pr-12'))}>
            {tags && tags.length > 0 && (
              <View style={tw(cleanStyle('flex flex-row items-center'))}>
                {tags.map((tag, index) => {
                  ...
                  return (
                    <View key={index} style={tw(cleanStyle(`flex flex-row items-center p-3 px-4 border-r w-1/3 justify-center border-neutral-300 ${tagColor} ${lastStyles}`))}>
