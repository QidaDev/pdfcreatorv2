...
const LinkList = ({ 
  ...
  const renderLink = (link: { text: string; linkSrc: string; linkText: string}, index: number, isFirst: boolean = false ) => {
    return (
      <View key={index}>
        ...
        <View wrap={false} style={tw('flex flex-row items-center justify-between text-[13px] p-3')}>
          <View style={tw('w-full flex flex-row items-center text-black')}>
          ...
          <View style={tw('ml-5 w-auto')}>
            <Link src={link.linkSrc} style={tw(`text-neutral-900 items-center font-normal flex flex-row w-auto`)}>
              <Text style={tw('mr-3 -mt-1 block')}>{renderText(link.linkText)}</Text>
              <View style={tw('ml-3')}>
  ...
  return (
    <View style={tw(`mb-${marginBottom} mt-${marginTop} ml-${marginLeft} mr-${marginRight}`)}>
      ...
      <View style={tw('flex flex-col')}>
        {listWithoutFirstLink.map((link, index) => (
